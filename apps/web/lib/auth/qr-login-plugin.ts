...
import {verifyExchangeToken, verifyMobileLoginToken} from '@/app/api/auth/qr-login/utils'
...
export const qrLoginPlugin = () => {
	return {
		...
		endpoints: {
			 ...
			 */
			mobileLoginExchange: createAuthEndpoint(
				'/qr-login/mobile-exchange',
				{
					method: 'POST'
				},
				async (c) => {
					...
					const userId = tokenData.userId
					context.logger?.info?.('Mobile Login exchange attempt', {userId})
				...
				context.logger?.info?.('Mobile Login session created', {
					sessionId: session.id,
					userId: session.userId
				})
