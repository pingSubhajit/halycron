 ...
 * Enables session creation via QR code scanning from mobile app (mobile -> web flow)
 * Based on: https://github.com/better-auth/better-auth/discussions/2125
 *
 * Note: The web -> mobile flow (mobileLoginExchange) is handled by a standalone
 * Next.js API route at /api/auth/qr-login/mobile-exchange to bypass CSRF checks
 * which block requests without Origin headers (like mobile apps)
 */
export const qrLoginPlugin = () => {
	return {
		id: 'qr-login',
		endpoints: {
			qrLoginExchange: createAuthEndpoint(
				...
				async (c) => {
					...
					const sessionData = await context.adapter.create({
						model: 'session',
						data: {
							userId,
							token: sessionToken,
