...
export const middleware = async (request: NextRequest) => {
	...
	} else if (path.startsWith('/api/auth/qr-login/initiate')) {
		rateLimitResult = await rateLimit(request, RATE_LIMIT_CONFIGS.qrLoginInitiate, 'qr-initiate')
	} else if (path.startsWith('/api/auth/qr-login/status')) {
		rateLimitResult = await rateLimit(request, RATE_LIMIT_CONFIGS.qrLoginStatus, 'qr-status')
	} else if (path.startsWith('/api/auth/qr-login')) {
		rateLimitResult = await rateLimit(request, RATE_LIMIT_CONFIGS.standard, 'qr-login')
