...
Halycron’s key management is designed to achieve **zero-knowledge at rest** while preserving:

- password reset without data loss
- a familiar sharing UX (open link → optional PIN → view)
- session creation without requiring a password (e.g., QR login), with vault unlock handled separately
